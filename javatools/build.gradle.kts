...
dependencies {
    ...
    // Kotlin stdlib — provided by org.xtclang.build.java convention plugin
    ...
    from(configurations.compileClasspath.map { cfg ->
        cfg.filter { it.name.endsWith(".jar") }.map { file ->
            zipTree(file).matching {
                ...
                // Exclude SLF4J - already present in javatools deps, avoid duplicate class errors
                exclude("org/slf4j/**")
                // Exclude kotlin module metadata from TrikeShed, not needed in fat jar
                exclude("META-INF/org.bereft_TrikeShed.kotlin_module")
                // Exclude kotlin stdlib from TrikeShed's transitive deps (in fat jar only; compileOnly at runtime)
                exclude("kotlin/**")
            }
        }
    })
