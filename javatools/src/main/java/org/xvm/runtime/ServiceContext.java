...
public class ServiceContext {
    ...
    // ── Pointcut hook — lib_cursor registers implementations at class load ──
    public interface PointcutHook {
        boolean active();
        void publish(int opcode, String method, int addr);
        void fieldPublish(int opcode, String method, int addr, boolean after);
    }

    /** No-op hook when lib_cursor is not on classpath. */
    private static final PointcutHook NOOP_POINTCUT = new PointcutHook() {
    ...
    /** lib_cursor sets this to wire VmPointcutPublisher + FieldSynapse. */
    public static volatile PointcutHook pointcut = NOOP_POINTCUT;
        ...
        while (true) { // main loop
            while (iPC >= 0) { // most common op return loop
                ...
                try {
                        ...
                        try {
                            ...
                            if (op != null && frame.f_function != null) {
                                ...
                                if (opCode >= 0xA5 && opCode <= 0xA8) {
                                    pointcut.fieldPublish(opCode, methodName, iPC, false);
                                    fieldOpCode = opCode;
                                }
                            }
                    ...
                    // pointcut: AFTER hooks for field synapse
                    if (fieldOpCode >= 0 && pointcut.active()) {
                        try {
                            String methodName = frame.f_function.getIdentityConstant().getName();
                            pointcut.fieldPublish(fieldOpCode, methodName, iPCLast, true);
                        } catch (Exception ignored) {
                        }
                    }
                } catch (Throwable e) {
